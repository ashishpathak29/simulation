...
function SetPageNameLoadPage ( pagenamestr )
{
   var status = doLMSGetValue( "cmi.core.lesson_status" );
	if (status == "not attempted")
	{
	...
	// Set the page name in cmi.suspend_data attribute...
	// the student is now attempting the lesson
	doLMSSetValue( "cmi.suspend_data", pagenamestr );
	exitPageStatus = false;
}

function GetPageNameLoadPage()
{
...
function computeTime(formattedTime)
{
	
	if (cursorLocation == "TOC")
	{
		startDate = window.parent.startDate
	} else if (cursorLocation == "IndexOrTest")
	{
		startDate = SCOStartTime
	}else{
		//startDate = window.parent.startDate
	}
   if ( startDate != 0 )
...
//tvs
function doSetLMSStatus( status )
{
...
/*******************************************************************************
** Added by Sham-08122003: The function would be called on the unload of every
** HTML to set a corresponding status to the Module of the course with the following
** factors in mind, though,
**		1. If the course (SCO) has a 'completed' status, no change is done to the 
**		   'cmi.core.lesson_status' attribute.
**		2. If the course (SCO) has any other status, the specified status is set to 
**		   'cmi.core.lesson_status' attribute.
*******************************************************************************/
function SetLessonStatus ( status ) {
	var CurrentLessonStatus = doLMSGetValue( "cmi.core.lesson_status" );
	if (CurrentLessonStatus == "completed")
	{
		//alert("hi");
		/*if ( CurrentLessonStatus != 'completed' ) {
			if (exitPageStatus != true) {
				//doQuit( status );			//tvs commented
				doSetLMSStatus( status );	// tvs inserted
			}
		} else {
			if (exitPageStatus != true) {
				//doQuit( completed );			//tvs commented
				doSetLMSStatus( "completed" );	// tvs inserted
			}
		}*/
		doLMSSetValue("cmi.core.lesson_status",CurrentLessonStatus)
	}else{
		doLMSSetValue("cmi.core.lesson_status",status)
	}
}
// Added by Sham-12/19/2003: Tracking of when do a DoLMSFinish() API call...
var bContDoFinish = true

function CheckContLMSFinishCall() {
	//if ( bContDoFinish ) {
		doLMSFinish();
	//}
}
...
// Added by Sham-12/19/2003: Tracking of when do a DoLMSFinish() API call...
var bMenuDoFinish = true

function CheckMenuLMSFinishCall() {
	if ( bMenuDoFinish ) {
		doLMSFinish();
	}
}
...
// Added by Sham-12/19/2003: Tracking of when do a DoLMSFinish() API call...
var bGlosDoFinish = true

function CheckGlosLMSFinishCall() {
	if ( bGlosDoFinish ) {
		doLMSFinish();
	}
}
